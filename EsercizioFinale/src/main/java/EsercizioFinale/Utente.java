...
@Entity
@Table(name = "utente", schema = "codicicatastali")
public class Utente {
	@Id
	@GeneratedValue
	private int id;
	...
	public String toString() {
		return "Utente [id=" + id + ", nome=" + nome + ", cognome=" + cognome + ", dataNascita=" + dataNascita
				+ ", comuneNascita=" + comuneNascita + ", sesso=" + sesso + ", codiceFiscale=" + codiceFiscale + "]";
	}
	
	public String calcolaCF(String cognomecf,String nomecf,String dataNascitacf, String comune,String sesso) {
		String codFis="";
	    //calcolo le prime 3 lettere 
	    int cont=0;
	    cognomecf=cognomecf.toUpperCase();
	    nomecf=nomecf.toUpperCase();
	 
	    //caso in cui il cognome sia minore di 3 lettere
	    if (cognomecf.length()<3){
	        codFis+= cognomecf;
	        while (codFis.length()<3) codFis+= "X";
	        cont=3;
	    }
	    //caso cognome lunghezza normale
	    ...
	    //Comune di nascita con codici catastali dal DBMySQL
	   
	    Connection c;
		try {
			c = DriverManager.getConnection("jdbc:mysql://localhost/codicicatastali?useSSL=false","root","mysqlAdmin");
			Statement s = (Statement) c.createStatement ();
	        ResultSet comuni =s.executeQuery("SELECT codFisco FROM codicicatastali.comuni WHERE comune ='"+comune+"'");
	        if (comuni.next()) {
